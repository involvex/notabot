...
export const allfilesCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<MessageActionReturn> => {
    ...
    if (arg === 'true' || arg === 'on' || arg === '1' || arg === 'yes') {
      ...
      const currentFileFiltering =
        context.services.settings.merged.fileFiltering;
      const isCurrentlyRestrictive =
        currentFileFiltering?.respectGitIgnore ?? true;
    ...
    try {
      ...
      context.services.settings.setValue(
        SettingScope.User,
        'fileFiltering',
        updatedFileFiltering,
      );

      // Save the settings
      const userSettingsFile = context.services.settings.forScope(
        SettingScope.User,
      );
